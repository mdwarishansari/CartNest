...
const ProductDetail = () => {
  ...
  if (error) {
    return (
      <ErrorState
        ...
        message="We encountered an error loading the product details. Please check your internet connection and try again."
  ...
  if (!product) {
    return (
      <div className="bg-cream-paper min-h-[60vh] flex flex-col items-center justify-center p-8 text-center font-graphik">
        <div className="p-4 bg-pure-white border border-ash rounded-full mb-4">
          <Eye className="w-8 h-8 text-smoke" />
        </div>
        <h2 className="text-heading-sm font-nantes text-ink-black mb-2">Product Not Found</h2>
        <p className="text-body text-smoke max-w-md mb-6">
          The product you are looking for does not exist, has been removed, or is no longer available.
        </p>
        <Link
          to="/search"
          className="px-6 py-2.5 bg-ink-black text-pure-white text-caption font-semibold rounded-md hover:bg-charcoal transition-all"
        >
          Back to Products
        </Link>
      </div>
