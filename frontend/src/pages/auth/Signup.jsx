...
const Signup = () => {
  ...
  const handleGoogle = async () => {
    try {
      ...
      toast.success('Welcome to CartNest!');
      navigate('/');
    } catch (err) {
    ...
    <div className="bg-cream-paper min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12 font-graphik">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-heading font-nantes text-ink-black mb-1">Create Account</h1>
          <p className="text-caption text-smoke">Join CartNest to start shopping</p>
        </div>
        ...
        <div className="bg-pure-white rounded-md border border-ash p-7 sm:p-8">
          <form onSubmit={handleSignup} className="space-y-5">
            <div>
              <label className="block text-caption font-semibold text-charcoal mb-2">Full Name</label>
              <div className="relative">
                <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-smoke" />
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className={inputCls} placeholder="John Doe" required autoFocus />
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-smoke mt-1.5">Must be at least 6 characters</p>
            </div>

            <button type="submit" disabled={loading} className="w-full py-2.5 px-6 bg-ink-black text-pure-white font-semibold rounded-md hover:bg-charcoal transition-all disabled:opacity-60 disabled:cursor-not-allowed mt-2 text-caption cursor-pointer">
              {loading ? (
          ...
          {/* Google */}
          <button onClick={handleGoogle} className="w-full py-2.5 px-6 bg-pure-white border border-ash rounded-md text-caption font-semibold text-charcoal hover:bg-cream-paper/20 hover:border-charcoal transition-all flex items-center justify-center gap-3 cursor-pointer">
          ...
          <div className="mt-7 space-y-2 text-center text-caption">
            <p className="text-smoke">Already have an account? <Link to="/auth/login" className="text-charcoal font-semibold hover:text-ink-black hover:underline">Sign In</Link></p>
            <p className="text-xs text-smoke">Want to sell? <Link to="/seller/register" className="text-smoke font-semibold hover:text-ink-black hover:underline">Register as Seller</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
